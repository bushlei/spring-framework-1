...
To retrieve a list of `com.zero.st002.User` objects in Java, you would normally write the following:

[source,java,indent=0]
----
	Flux<com.zero.st002.User> users  = client.get().retrieve().bodyToFlux(com.zero.st002.User.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<com.zero.st002.User>()
	// or (both are equivalent)
	val users : Flux<com.zero.st002.User> = client.get().retrieve().bodyToFlux()
----
...
NOTE: Spring Boot is based on com.zero.st002.JavaConfig and
...
class UserHandler(builder: WebClient.Builder) {
	...
	suspend fun listView(request: ServerRequest): ServerResponse =
			...
			client.get().uri("...").awaitExchange().awaitBody<com.zero.st002.User>()))
...
* `equals()` and `hashCode()`
* `toString()` of the form `"com.zero.st002.User(name=John, age=42)"`
